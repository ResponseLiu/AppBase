...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        self.backgroundColor = [UIColor whiteColor]; self.hisIdx = 44;
...
-(void)InitNormalMenu:(NSArray *)arr{
    [arr enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.isLine) { [self.lineArrM addObject:[JCLKitObj JCLView:self color:JCL_Line_COL]]; }
    }];
}
...
-(void)InitBGLineMenu:(NSArray *)arr{
    ...
    self.bgLine = [JCLKitObj JCLView:self color:JCLRGB(220, 220, 220)];
}
...
-(void)layoutNormalMenu:(CGFloat)height{
    CGFloat lineW = 1, lineH = 0.6*height, lineY = 0.5*(height - lineH);
    ...
    [self.arrM enumerateObjectsUsingBlock:^(JCLKitModel *_Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (self.isAction) {
            if (idx == self.idx) {
                ...
                obj.text.textColor = JCL_SelText_COL;
                self.select = obj;
...
-(void)layoutIdxLineMenu:(CGFloat)height{
    ...
    self.idxLine.frame = CGRectMake(0.5*(w - 1.2*size.width)+self.idx*w, h, 1.4*size.width, 1.4);
